...
function dockerBuildAndPush() {
  ...
  echo docker buildx build --file ${1}/Dockerfile --platform=linux/arm64/v8,linux/amd64 $(getTagsArg ${ECR_URL}) --provenance=false --push
  echo ========================
  docker buildx build --file ${1}/Dockerfile --platform=linux/arm64/v8,linux/amd64 $(getTagsArg ${ECR_URL}) --provenance=false --push .
  echo ========================
...
dockerBuildAndPush "packages/${service}" "${ECR_URL}-${service}"

echo ========================
echo 🎉 Released to ECR: $TAGS
